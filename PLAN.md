...
> **Status: in progress.** Slices 5.1 (the month-grouped timeline), 5.2 (tags), and
> 5.3 (full-text search — the generated `search` tsvector + GIN index, global ranked
> search across the workspace, deep-linking) are done and committed. Remaining:
> timeline filters (tag/category/date), cursor pagination, the expand-to-detail
> view, and responsive/keyboard + loading/empty/error polish.
