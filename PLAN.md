...
- [x] Tests: role-gating on every admin action; member cannot reach admin routes — _slice 2.3: a pure `roles` policy (`canAdminister` / `canManageMember` / `canTransferOwnershipTo`) is unit-tested and drives the admin gate + members UI; the actions enforce it via `requireRole` and Better Auth. Verified live that a member is redirected off `/admin`. (Per-action integration tests await a DB-in-CI setup; the Vitest job is intentionally DB-free.)_
