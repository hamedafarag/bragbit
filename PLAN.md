...
- [ ] Branding applied across login, app chrome, share pages, and all email templates; "Powered by BragBit" footer on share pages — _slice 2.1 applied accent + logo + name to the app chrome and the login page (via a per-workspace `--primary`/`--ring` override on the layout wrapper). Email templates land with the members/invite slice (2.2); share pages in Phase 6._
- [ ] Personal-workspace UX: hide member/invite/role chrome; expose profile + light personalization (display name, avatar, optional accent) — useful for a freelancer's client-facing share pages — _slice 2.1: a personal-workspace owner can set name + accent + logo from `/admin`; profile + avatar shipped in Phase 1. Member/invite chrome hiding verifies alongside the members UI in slice 2.2._
- [ ] Members management (organizations): list with roles + last activity; invite (single + multiple); pending invitations (resend/revoke); change role; **remove member = export-then-delete** (member gets a portable Markdown + JSON + attachments bundle, then their data is purged from the workspace; full offboard in `private-org`)
- [ ] Ownership transfer (owner only); admins can never demote/remove the owner
- [ ] Tests: role-gating on every admin action; member cannot reach admin routes
...
- [ ] Impact field, category select, status, collaborators, attribution (recognition brags)
- [ ] Multiple links per brag with labels
- [ ] Markdown editor for description/impact (edit/preview), sanitized rendering
- [ ] Empty-state onboarding: *"Start by back-filling three wins from the past month"*
